...
CREATE TABLE `ims_ewei_shop_member` (
  ...
  `diycrowdid` int(11) NOT NULL COMMENT '众筹diy编号',
  `diycrowdfields` text NOT NULL COMMENT '众筹数据',
  `diycrowddata` text NOT NULL COMMENT '众筹数据',
  `iscrowd` tinyint(1) NOT NULL COMMENT '是否申请了众筹1是',
  `crowdstatus` tinyint(1) NOT NULL COMMENT '众筹申请状态',
  `crowdtime` int(10) NOT NULL COMMENT '众筹通过时间',
  `crowdblack` tinyint(1) NOT NULL COMMENT '是否众筹黑名单1是',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_level` (`level`)
) ENGINE=MyISAM AUTO_INCREMENT=20480 DEFAULT CHARSET=utf8;

-- ----------------------------
-- Table structure for userbases
-- ----------------------------
DROP TABLE IF EXISTS `userbases`;
CREATE TABLE `userbases` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `userbasestatus` varchar(10) DEFAULT NULL,
  ...
  `suggestorusercard` varchar(20) DEFAULT NULL,
...
CREATE TABLE `yzt_gxc_user` (
  ...
  `cardsn` varchar(12) DEFAULT NULL COMMENT '卡号',
...
) ENGINE=InnoDB AUTO_INCREMENT=18617 DEFAULT CHARSET=utf8 COMMENT='用户';
...
) ENGINE=InnoDB AUTO_INCREMENT=18612 DEFAULT CHARSET=utf8;
